#include "percolation_graph_io.hpp"
...
myint mergeSortDec(myint *sequenceToBeSorted,
                   myint lengthOfTheSequenceToBeSorted,
                   myint *alreadySortedSequence = NULL,
                   myint lengthOfSortedSequence = 0) {
...
myint mergeSortDecBlock(myint *unsortedS,
                        myint lengthOfUnsorted,
                        myint blockSize,
                        myint *alreadySorted = NULL,
                        myint lengthOfSorted = 0) {
                    ...
                    if (unsortedS[rL * blockSize + 1] > alreadySorted[rR * blockSize + 1]) {
                        ...
                        if (unsortedS[rL * blockSize + 1] == alreadySorted[rR * blockSize + 1]) {
                            if (unsortedS[rL * blockSize + 3] > alreadySorted[rR * blockSize + 3]) {
                                ...
                                if (unsortedS[rL * blockSize + 3] == alreadySorted[rR * blockSize + 3]) {
                                    if (unsortedS[rL * blockSize + 2] - unsortedS[rL * blockSize + 4]
                                        > alreadySorted[rR * blockSize + 2] - alreadySorted[rR * blockSize + 4]) {
                                        comparisonInd = 1;
                                    }
                                }
                            }
...
myint CreateMemObjects(cl_context context,
                       cl_mem *memObjects,
                       Graph *myGraph,
                       myint *parameters,
    ...
    while (begV != endV) {
        ...
        for (tie(begE, endE) = adjacent_vertices(*begV, *myGraph); begE != endE; ++begE) {
            ...
            tempName2 = get(&VertexProperties::vName, *myGraph, *begE);
            if ((get(&VertexProperties::vState, *myGraph, *begV) == 1)
                || (get(&VertexProperties::vState, *myGraph, *begE) == 1)) {
...
mysint changeParameters(cl_context *context,
    ...
    errNum = clEnqueueWriteBuffer(*commandqueue,
                                  ...
                                  CL_TRUE,
                                  0,
                                  sizeof(myint) * numberOfParameters,
                                  parameters,
                                  0,
                                  NULL,
                                  &eventE);
...
myint checkTerminalCondition(cl_context *context,
                             cl_command_queue *commandQueue,
                             cl_program *program,
                             cl_kernel *kernfls,
                             myint numKernels,
...
myint mergeSortDecParallel(cl_context *context,
                           cl_command_queue *commandQueue,
                           cl_program *program,
                           cl_kernel *kernfls,
                           myint numKernels,
...
myint mergeSortDecBlockParallel(cl_context *context,
                                cl_command_queue *commandQueue,
                                cl_program *program,
                                cl_kernel *kernfls,
                                myint numKernels,
...
myint cleanSequence_parallel(cl_context *context,
                             cl_command_queue *commandQueue,
                             cl_program *program,
                             cl_kernel *kernfls,
                             myint numKernels,
...
myint cleanPhantomSequence_parallel(cl_context *context,
                                    cl_command_queue *commandQueue,
                                    cl_program *program,
                                    cl_kernel *kernfls,
                                    myint numKernels,
    ...
    mergeSortDecBlockParallel(context,
                              commandQueue,
                              program,
                              kernfls,
                              numKernels,
...
myint cleanSequence(myint *sequence,
                    myint *sequenceLength,
                    myint *storageStart,
                    myint *cleanFrom,
                    myint cleaningLength,
                    myint removalSign) {
...
myint cleanPhantomSequence(myint *sequence,
                           myint *sequenceLength,
                           myint *storageStart,
                           myint *cleanFrom,
                           myint cleaningLength,
            ...
            if (seq[position2 * blockSize + 1] > biggerTerm[1]) {
                ...
                if (seq[position2 * blockSize + 1] == biggerTerm[1]) {
                    if (seq[position2 * blockSize + 3] > biggerTerm[3]) {
                        ...
                        if (seq[position2 * blockSize + 3] == biggerTerm[3]) {
                            if (seq[position2 * blockSize + 2] - seq[position2 * blockSize + 4]
                                < biggerTerm[2] - biggerTerm[4]) {
                                for (cnter = 0; cnter < blockSize; cnter++) {
            ...
            if (auxSeq[(currentPosition - 1) * blockSize + 1] > biggerTerm[1]) {
                ...
                if (auxSeq[(currentPosition - 1) * blockSize + 1] == biggerTerm[1]) {
                    if (auxSeq[(currentPosition - 1) * blockSize + 3] > biggerTerm[3]) {
                        ...
                        if (auxSeq[(currentPosition - 1) * blockSize + 3] == biggerTerm[3]) {
                            if (auxSeq[(currentPosition - 1) * blockSize + 2]
                                - auxSeq[(currentPosition - 1) * blockSize + 4] < biggerTerm[2] - biggerTerm[4]) {
                                for (cnter = 0; cnter < blockSize; cnter++) {
...
myint algCycle(cl_context *context,
               cl_command_queue *commandQueue,
               cl_program *program,
               cl_kernel *kernfls,
               myint numKernels,
               ...
               size_t pws,
               myint numMemObjects,
               myint *activeVerticesSeq,
               myint *activeEdgesSeq,
               myint *phantomSeq) {
    ...
    while (fCounter < numberOfReadyEdges) {
        ...
        if (nThreadsTemp > thresholdForParallel) {
            ...
            cleanSequence_parallel(context,
                                   commandQueue,
                                   program,
                                   kernfls,
                                   numKernels,
            ...
            errNum = clEnqueueReadBuffer(*commandQueue,
                                         memObjects[4],
                                         CL_TRUE,
                                         parameters[9] * sizeof(myint),
                                         (parameters[32] + nThreadsTemp - parameters[9]) * sizeof(myint),
                                         activeVerticesSeq + parameters[9],
                                         0,
                                         NULL,
                                         &eventE);
            ...
            cleanSequence(activeVerticesSeq,
                          parameters + 11,
                          parameters + 9,
                          parameters + 32,
                          nThreadsTemp,
            ...
            clEnqueueWriteBuffer(*commandQueue,
                                 ...
                                 CL_TRUE,
                                 parameters[9] * sizeof(myint),
                                 sizeof(myint) * (paramSaver2 + nThreadsTemp - parameters[9]),
                                 activeVerticesSeq + parameters[9],
                                 0,
                                 NULL,
                                 &eventE);
    ...
    while (fCounter < numberOfTriggeredVertices) {
        ...
        if (nThreadsTemp * dimension * 2 > thresholdForParallel2) {
            ...
            errNum = clEnqueueReadBuffer(*commandQueue,
                                         memObjects[5],
                                         CL_TRUE,
                                         parameters[10] * sizeof(myint),
                                         (parameters[34] + nThreadsTemp * dimension * 2 - parameters[10])
                                             * sizeof(myint),
                                         activeEdgesSeq + parameters[10],
                                         0,
                                         NULL,
                                         &eventE);
            ...
            cleanSequence(activeEdgesSeq,
                          parameters + 12,
                          parameters + 10,
                          parameters + 34,
                          nThreadsTemp * dimension * 2,
                          parameters[20]);
            ...
            clEnqueueWriteBuffer(*commandQueue,
                                 ...
                                 CL_TRUE,
                                 parameters[10] * sizeof(myint),
                                 sizeof(myint) * (paramSaver2 + nThreadsTemp * dimension * 2 - parameters[10]),
                                 activeEdgesSeq + parameters[10],
                                 0,
                                 NULL,
                                 &eventE);
    ...
    while (fCounter < numberOfTriggeredEdges) {
        ...
        if (paramSaver2 > thresholdForParallel2) {
            parameters[4] = nThreadsTemp;
            cleanPhantomSequence_parallel(context,
                                          commandQueue,
                                          program,
                                          kernfls,
                                          numKernels,
            ...
            parameters[29] = 0;
            cleanPhantomSequence(phantomSeq,
                                 parameters + 30,
                                 parameters + 29,
                                 parameters + 36,
                                 nThreadsTemp,
    ...
    if (cleaningLength1 > thresholdForParallel2) {
        ...
        errNum = clEnqueueReadBuffer(*commandQueue,
                                     ...
                                     CL_TRUE,
                                     parameters[10] * sizeof(myint),
                                     (paramSaver1 - parameters[10]) * sizeof(myint),
                                     activeEdgesSeq + parameters[10],
                                     0,
                                     NULL,
                                     &eventE);
        ...
        clEnqueueWriteBuffer(*commandQueue,
                             ...
                             CL_TRUE,
                             parameters[10] * sizeof(myint),
                             sizeof(myint) * (paramSaver1 - parameters[10]),
                             activeEdgesSeq + parameters[10],
                             0,
                             NULL,
                             &eventE);
    ...
    checkEnd = checkTerminalCondition(context,
                                      commandQueue,
                                      program,
                                      kernfls,
                                      numKernels,
...
myint shortestPathParallel(Graph *myGraph) {
    ...
    *numberOfParameters = 61;
    ...
    parameters[29] = 0;
    parameters[37] = -71;
    parameters[20] = defaultNumberForEmpty;
    getSetupFromFile("setup.txt",
                     parameters + 17,
                     parameters + 7,
                     &newDim,
                     parameters + 26,
    ...
    tBeg = time(0);
    myint counterCycl = 0;
    myint algorithmChoice = parameters[42];
    myint memoryCreationTime = 0;
